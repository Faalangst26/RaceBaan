...
namespace WPF2
{
    public static class VisualizeWPF
    {
        ...
        private static Point cursorpos = new Point(0, 0);
        public static BitmapSource DrawTrack(Track track)
        {
            Bitmap trackimg = new Bitmap(ImageHandler.GetBackground(200, 200));
            ...
            foreach (Section section in track.Sections)
            {
                ...
                if (section.SectionType.Equals(SectionTypes.RightCorner)){
                    _direction++;
                    gr.DrawImage(Image.FromFile(corner_right), cursorpos);
               }
                //Bochten naar links
                if (section.SectionType.Equals(SectionTypes.LeftCorner)){
                    _direction--;
                    gr.DrawImage(Image.FromFile(corner_left), cursorpos);
                }
